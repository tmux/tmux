...
# Test 6: Content should not appear on screen until sync ends
$TMUX -f/dev/null new -d -x40 -y10 || exit 1
sleep 1
...
if grep -q "SYNCTEST" "$TMP"; then
	echo "[FAIL] Test 6: Content appeared before sync ended"
	exit_status=1
else
	echo "[PASS] Test 6: Content buffered during sync mode"
fi

# Now capture again - content should be visible
$TMUX capturep -p > $TMP
if grep -q "SYNCTEST" "$TMP"; then
	echo "[PASS] Test 7: Content visible after sync ended"
else
	echo "[FAIL] Test 7: Content not visible after sync ended"
	exit_status=1
fi
...
# Test 8: Content should appear after timeout (timer fires)
$TMUX -f/dev/null new -d -x40 -y10 || exit 1
sleep 1
...
if grep -q "TIMERTEST" "$TMP2"; then
	echo "[FAIL] Test 8a: Content appeared before timeout"
	exit_status=1
else
	echo "[PASS] Test 8a: Content buffered during sync mode"
fi
...
if grep -q "TIMERTEST" "$TMP2"; then
	echo "[PASS] Test 8b: Content visible after timeout"
else
	echo "[FAIL] Test 8b: Content not visible after timeout"
	exit_status=1
fi
