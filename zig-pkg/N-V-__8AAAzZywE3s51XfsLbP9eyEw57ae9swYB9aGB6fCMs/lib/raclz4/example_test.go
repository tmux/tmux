...
// raclz4.CodecWriter) and decompressing (using a rac.Reader and a
// raclz4.CodecReader). This includes decompressing an excerpt of the original
...
func Example_roundTrip() {
	...
	w := &rac.Writer{
		Writer:      cBuf,
		CodecWriter: &raclz4.CodecWriter{},
	...
	// The exact compression ratio depends on the lz4 encoder's algorithm,
	// which can change across Go standard library releases, but it should be
	// at least a 3x ratio. It'd be larger if we didn't specify an explicit
	// (but relatively small) DChunkSize.
	if ratio := len(original) / len(compressed); ratio < 3 {
	...
	r := &rac.Reader{
		...
		CodecReaders:   []rac.CodecReader{&raclz4.CodecReader{}},
	}
	defer r.Close()
